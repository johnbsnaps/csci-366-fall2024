...
when "grade"
  ...
  when "emulator"
    for_each_student_dir do |first, last, dir|
      ...
      pull
      make_grading_dir
      if not Dir.exist? 'lmsm'
        next
      end
      Dir.chdir 'lmsm' do
        cmake "../grading/project_1_results.txt"
        maybe_exec "timeout 30 ./test/lmsm_emulator_tests", "../grading/project_1_results.txt"
      end
      push_grading
    end
  when "final"
    for_each_student_dir do |first, last, dir|
      ...
      Dir.chdir 'lmsm' do
        cmake "../grading/project_1_results.txt"
        maybe_exec "timeout 30 ./test/lmsm_tests", "../grading/project_final_results.txt"
      end
      push_grading
    end
  when "assembler"
